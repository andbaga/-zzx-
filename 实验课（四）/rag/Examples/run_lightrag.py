...
async def llm_model_func(
    ...
    **kwargs
    ...
    # Get API configuration from kwargs (use pop to avoid duplicate kwargs)
...
async def initialize_rag(
    ...
    if mode == "API":
        ...
        llm_model_func_final = llm_model_func
    elif mode == "ollama":
        embedding_func = EmbeddingFunc(
            embedding_dim=1024,
            max_token_size=8192,
...
async def process_corpus(
    ...
    # Index the corpus content (Truncated to ~50 chunks for faster testing)
    truncated_context = context[:int(len(context) * 50 / 199)]
    rag.insert(truncated_context)
    logging.info(f"✅ Indexed truncated corpus: {corpus_name} ({len(truncated_context.split())} words, approx 50 chunks)")
    ...
    for q in tqdm(corpus_questions, desc=f"Answering questions for {corpus_name}"):
        ...
        context = []  # LightRAG v1.4.9 does not return context separately
